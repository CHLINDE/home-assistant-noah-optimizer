...
ENTITY_TOKENS: dict[str, tuple[str, str]] = {
    ...
    "__CONTROLLER_STATUS__": ("sensor", "controller_status"),
...
def _patch_controller_status_display(
    content: str,
    controller_status_entity: str,
) -> tuple[str, bool]:
    """Use the translated controller-status enum sensor in stored cards."""
    # Only migrate cards that still contain the standard legacy controller
    # line. This avoids removing a status map from a user-customized card
    # that may use it for other content.
    if not any(
        "**Controller:**" in line and "status_text.get(" in line
    ...
    lines = content.splitlines(keepends=True)
    patched: list[str] = []
    skip_status_map = False

    for line in lines:
        if "{% set status = state_attr(ctrl, 'control_status') %}" in line:
            changed = True
            continue

        if "{% set status_text = {" in line:
            skip_status_map = True
            changed = True
            continue
        ...
        if "**Controller:**" in line and "status_text.get(" in line:
            line_without_eol = line.rstrip("\r\n")
            eol = line[len(line_without_eol):]
            indent_length = (
                len(line_without_eol)
                - len(line_without_eol.lstrip())
            )
            indent = line_without_eol[:indent_length]
            trailing = "  " if line_without_eol.endswith("  ") else ""
            line = (
                f"{indent}**Controller:** "
                f"{{{{ state_translated('{controller_status_entity}') }}}}"
                f"{trailing}{eol}"
            )
            changed = True
...
def _patch_status_markdown(
    ...
    if "'pv_redirect'" not in content:
        if german and "'soc_release': 'SOC-Freigabe'" in content:
            content = content.replace(
                "'soc_release': 'SOC-Freigabe'",
                "'soc_release': 'SOC-Freigabe',\n"
            ...
            changed = True
        elif not german and "'soc_release': 'SOC release'" in content:
            content = content.replace(
                "'soc_release': 'SOC release'",
                "'soc_release': 'SOC release',\n"
                "                'pv_redirect': 'PV diversion'",
            )
            changed = True

    controller_status_entity = replacements["__CONTROLLER_STATUS__"]
    content, controller_status_changed = _patch_controller_status_display(
        content,
        controller_status_entity,
    )
    changed |= controller_status_changed
